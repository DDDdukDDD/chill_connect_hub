...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  // Mode: 'login' | 'signup-social' | 'signup-email'
  const [mode, setMode] = useState<'login' | 'signup-social' | 'signup-email'>(
    initialMode === 'signup' ? 'signup-social' : 'login'
  );
  ...
  const isLoginFormValid = email.trim().length > 0 && email.includes('@') && password.trim().length > 0;
  ...
  // Password strength
  const passwordStrength = signUpPassword.length >= 10 && /[A-Z]/.test(signUpPassword) && /\d/.test(signUpPassword)
    ? 3
    ...
    <>
      <div className="fixed inset-0 z-[100002] flex items-center justify-center p-4 sm:p-6 bg-black/65 backdrop-blur-xs animate-fade-in">
        ...
        {mode === 'login' ? (
          ...
          /* VIEW 1: LOGIN CARD */
          /* ======================================================== */
          <div className="bg-white rounded-[32px] max-w-[500px] w-full border border-slate-200 shadow-2xl overflow-hidden p-7 sm:p-10 animate-scale-up relative">
            {/* Close Button X */}
            <button
              onClick={onClose}
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <ArrowRight className={`w-5 h-5 ${isLoginFormValid ? 'text-slate-900' : 'text-slate-300'}`} />
                </button>
              </div>
            </form>

            {/* Signup Link -> Switches to Signup Mode inside Modal */}
...
type="button"
            ...
            {/* Footer Login Link -> Returns to Login view inside modal */}
          ...
          /* ======================================================== */
          /* VIEW 3: FINISH SIGNING UP WITH EMAIL (SPACIOUS & BRANDED - HIDDEN SCROLLBAR) */
          /* ======================================================== */
            ...
            <form onSubmit={handleEmailSignUpSubmit} className="space-y-4">
              ...
              <div className="space-y-1 pt-1">
                ...
                <div className="flex items-center gap-2.5 pt-0.5">
                  <input
                    type="checkbox"
                    id="confirm-age-auth-modal"
                    checked={isAgeConfirmed}
                    onChange={(e) => setIsAgeConfirmed(e.target.checked)}
                    className="w-4.5 h-4.5 rounded text-[#4A7C59] focus:ring-[#4A7C59] cursor-pointer"
                  />
                  <label htmlFor="confirm-age-auth-modal" className="text-xs sm:text-sm text-slate-700 font-semibold cursor-pointer">
